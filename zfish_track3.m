...
for i = 1:size(y,3) % cycles through frames
 ...
 imshow(y(:,:,i)) % Displays refined filtered image for plotting over
 hold on
 for k = 1:n % Cycles through objects
    plot(centroids_data(k,1:2:(2*i - 1)),centroids_data(k,2:2:(2*i)),'g','LineWidth',4) % First term plots all x coordinates of the given fish while the second term plots all y coordinates of the given fish, forming a connected line for each fish individually
 end
